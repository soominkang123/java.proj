...
public class Member {  // 회원 정보를 담은 클래스
	...
	/*
	     접근 제어자 : OOP의 캡슐화 , 필드나 , 메소드 , 클래스에 보안기능
	             - 클래스 : public , default
	...
	// 1. 필드 : 외부 클래스에서 필드를 직접 접근 할 수 없도록 한다.
	 ...
	 long idx;      // 회원의 넘버링이 자동으로 됨   //alt shift a 다중선택
	private String id;     // ID
	private String pw;     // 패스워드를 저장
	private String name;   // 회원이름
	private int year;   // 년   : 1 ~ 12
	private int month;  // 월
	private int day;    // 태어난 일
   ...
   // 2. 생성자 
	...
	// 3. getter (출력) / setter(입력) 를 사용해서 필드의 값을 출력 / 입력 
	public long getIdx() {
		return idx;
	}
	...
	// toString() 메소드 Overriding :   
	   ...
	   // Overriding : 부모 클래스의 메소드를 자식 클래스에서 재정의 해서 사용 하는 것
