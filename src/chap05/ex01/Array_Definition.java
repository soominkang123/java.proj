		 ...
		 자료형 : 변수, 상수 , 배열(Array) 을 정의 할때 자료형이 정의 되어야 한다
		   - 기본 자료형 : Stack 공간 변수명, 값 , 값 비교   == , stack 의 값을 비교
		      ...
		      - 참조 자료형 :	Stack ( 변수, 참조주소 ) , Heap 에 값이 저장됨 . (변수명).equals() : Heap 의 값
		         ...
		         배열 (Array) : 하나의 배열 변수에 여러개의 값을 넣을수 있다.
		            - 방번호 ( index ) : 0 번 방부터 시작
...
//		System.out.println(b);  // 오류 발생 : 기본 자료형은 초기값이 할당 되지 않으면 오류 발생
		...
		int[] arr = new int [5] ;      // index : 0 , 1 , 2 , 3 , 4 ,  ★ 방의 번호를 index라 칭함
		   // 자료형[] 배열변수 = new int[방의갯수]
		      // new : 배열의 값을 Heap 에 저장 , 
		      // int [5] : 를 생성
		
		System.out.println("===배열 방의 값o를 출력======");
		// 2. 배열의 각 방의 값을 출력 
 ...
 //       arr[5] = 60 ;  // 실행시 오류 발생 : 런타임 오류( 실행시 오류발생 ) , 
                       // ArrayIndexOutOfBoundsException < == 배의의 방번을 넘긴 값을 넣거나
		
        //3. 배열 각 방에 값을 출력 ( 값을 수정후 출력 )
        System.out.println("arr 배열의 0번째 방의 값을 출력 : " + arr[0]);
		System.out.println("arr 배열의 1번째 방의 값을 출력 : " + arr[1]);
		System.out.println("arr 배열의 2번째 방의 값을 출력 : " + arr[2]);
		System.out.println("arr 배열의 3번째 방의 값을 출력 : " + arr[3]);
